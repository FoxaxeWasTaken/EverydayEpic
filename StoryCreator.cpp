...
ee::Story *ee::createStory(std::shared_ptr<ee::Character> heroPtr, std::shared_ptr<ee::Character> badGuyPtr)
{
    ...
    ee::Story *story44 = new ee::Story(std::string("Lors de la reparation du tracteur, vous\nremarquez le saboteur au loin qui revient !\nVoulez decider d'aller lui parler"), heroPtr->getDescription() + std::string(" with a farmer repairing a tractor and seeing someone running, in a realistic style"), std::string("Lui demander calmement pourquoi il fait ça"), std::string("L'agresser"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ...
    ee::Story *story47 = new ee::Story(std::string("Vous avez decide de sonner a la porte,\ncomment lui parlez-vous ?"), heroPtr->getDescription() + std::string(" with another farmer, in front of a doorway with someone else in the house, in a realistic style"), std::string("Lui parler calmement pour savoir pourquoi il a fait ça"), std::string("Lui demander de nous aider"), heroPtr, badGuyPtr, ee::storyType::INGAME);
