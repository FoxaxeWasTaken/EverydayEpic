...
ee::Story *ee::createStory(std::shared_ptr<ee::Character> heroPtr, std::shared_ptr<ee::Character> badGuyPtr)
{
    ...
    /* Ville */
        /* First part*/
    ee::Story *story1 = new ee::Story(std::string("Votre histoire commence dans la ville.\nPreferez vous allez faire les courses ou\na votre rendez-vous ?"), heroPtr->getDescription() + std::string(" in a town with a grocery store"), std::string("Faire ses courses"), std::string("Aller a votre RDV"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story2 = new ee::Story(std::string("Vous avez decide de vous rendre a votre\nrendez-vous, mais vous apercevez un banc,\nsouhaitez-vous vous arreter au banc,\nou continuer vers votre rendez-vous"), heroPtr->getDescription() + std::string(" in a street with a bench"), std::string("Aller a votre rendez-vous"), std::string("Vous arreter sur le banc"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story3 = new ee::Story(std::string("Vous avez decide d'aller faire vos\ncourses, mais il faut traverser la route,\npensez-vous qu'il est important de regarder\navant de traverser ?"), heroPtr->getDescription() + std::string(" who will cross the road"), std::string("Traverser en regardant"), std::string("Traverser sans regarder"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story4 = new ee::Story(std::string("Vous avez traverse sans regarder,\nc'est dommage, vous venez de vous faire\necraser par une camionette. Vous ferez\nattention la prochaine fois !"), heroPtr->getDescription() + std::string(" crossing the road who get shooted by a car"), std::string(""), std::string(""), heroPtr, badGuyPtr, ee::storyType::LOOSE);
    ee::Story *story5 = new ee::Story(std::string("Vous avez bien fait de regarder avant de\ntraverser, vous venez d'eviter de justesse\nla camionette qui roulait a toute allure !\nVoulez vous entrer dans le magasin, ou\ncommencer a courir ?"), heroPtr->getDescription() + std::string(" in front of a road, with a driving truck"), std::string("Entrer dans le magasin"), std::string("Se mettre a courir"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ...
    ee::Story *story16 = new ee::Story(std::string("Vous avez decide de faire un caprice et\nde fuir ! Voulez-vous aller marcher dans\nla ville, ou continuer a courir ?"), heroPtr->getDescription() + std::string(" running in the street"), std::string("Partir marcher dans la ville"), std::string("Continuer a courir"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story17 = new ee::Story(std::string("Vous avez decide de ne pas aller a votre\nrendez-vous, voulez vous aller dans un parc, ou prendre votre voiture ?"), heroPtr->getDescription() + std::string(" in the street, in front of a park in his car"), std::string("Aller au parc"), std::string("Prendre la voiture"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ...
    ee::Story *story89 = new ee::Story(std::string("Vous eteignez le depart de flamme et vous\nsauvez la ville d'un terrible incendie.\nVous etes un super-heros !"), heroPtr->getDescription() + std::string(" stopping fire in a garbage can"), std::string(""), std::string(""), heroPtr, badGuyPtr, ee::storyType::WIN);
    ...
    ee::Story *story_farm = new ee::Story(std::string("Votre histoire commence dans votre ferme.\nPreferez vous allez faire les recolter\ndes carottes ou nourrir vos animaux ?"), heroPtr->getDescription() + std::string(" in a farm, with carrots, cows and chickens"), std::string("Aller a votre champ de carottes"), std::string("Aller nourrir vos animaux"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story24 = new ee::Story(std::string("Vous avez decide de nourrir vos animaux.\nPreferez-vous nourrir vos vaches ou vos poules ?"), heroPtr->getDescription() + std::string(" in a farm with cows and chickens"), std::string("Aller nourrir les vaches"), std::string("Aller nourrir les poules"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ...
    ee::Story *story33 = new ee::Story(std::string("Vous leurs avez donne du fourage vert,\nexcellent choix. Que comptez vous faire ensuite ?"), heroPtr->getDescription() + std::string(" giving grass to eat to a cow"), std::string("Inspectez vos vaches"), std::string("Recoltez vos carottes"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story34 = new ee::Story(std::string("Donner de la farine de viande n'est pas\nle meilleur choix..., vous venez de tuer\nvos vaches et vous etes desormais endette"), heroPtr->getDescription() + std::string(" being sad with a dead cow next to him"), std::string(""), std::string(""), heroPtr, badGuyPtr, ee::storyType::LOOSE);
    ...
    ee::Story *story36 = new ee::Story(std::string("Vous avez decide d'aller voir Kiri.\nElle va parfaitement bien.\nQue souhaitez vous faire ?"), heroPtr->getDescription() + std::string(" checking a cow"), std::string("Allez voir Marguerite"), std::string("Allez recolter vos carottes"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story37 = new ee::Story(std::string("Vous avez decide d'aller voir Marguertie.\nElle est dans un etat critique. Que faites vous ?"), heroPtr->getDescription() + std::string(" checking a hill cow"), std::string("La soigner"), std::string("Allez recolter vos carottes"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story39 = new ee::Story(std::string("Vous avez choisi de la soigner et elle\nvous en est tres reconnaisante,\nvous etes son super-heros !"), heroPtr->getDescription() + std::string(" checking a hill cow"), std::string("La soigner"), std::string("Allez recolter vos carottes"), heroPtr, badGuyPtr, ee::storyType::WIN);
    ...
    ee::Story *story51 = new ee::Story(std::string("Votre histoire commence a la montagne.\nVoulez-vous rester au coin du feu,\nou partir en randonnee ?"), heroPtr->getDescription() + std::string(" in a moutain house"), std::string("Rester au coin du feu"), std::string("Partir en randonnee"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story52 = new ee::Story(std::string("Vous avez decide de rester au coin du feu.\nVous recevez un appel d'un ami, mais il y a egalement\nquelqu'un qui sonne a votre porte.\nQue voulez-vous faire ?"), heroPtr->getDescription() + std::string(" in a moutain house, next to a fireplace"), std::string("Repondre a l'appel"), std::string("Ouvrir la porte"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story53 = new ee::Story(std::string("Vous avez decroche l'appel.\nC'est un ami qui vous propose d'aller au bar.\nQue faites-vous ?"), heroPtr->getDescription() + std::string(" next to a fireplace, calling someone on his phone"), std::string("Accepter"), std::string("Refuser, et aller ouvrir la porte a la personne qui a sonne"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story54 = new ee::Story(std::string("Vous avez decide d'aller ouvrir la porte.\nUn voisin vous propose une raclette.\nVous decidez d'y aller. Voulez-vous beaucoup manger ?"), heroPtr->getDescription() + std::string(" talking to someone else in front of his door"), std::string("Beaucoup manger"), std::string("Manger peu"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story56 = new ee::Story(std::string("Vous avez accepte d'aller au bar,\nVoulez-vous y aller a pieds ou en voiture ?"), heroPtr->getDescription() + std::string(" in front of a car in the mountains"), std::string("Y aller a pieds"), std::string("Y aller en voiture"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story57 = new ee::Story(std::string("Vous avez pris la voiture mais\nune chevre bloque la route. Que faire ?"), heroPtr->getDescription() + std::string(" in his car, blocked by a goat blocking the road"), std::string("S'en aller et aller au bar a pieds"), std::string("Essayer d'arreter la chevre"), heroPtr, badGuyPtr, ee::storyType::INGAME); //LIEN VERS 74 et 75
    ...
    ee::Story *story68 = new ee::Story(std::string("Vous fuyez mais la chevre vous poursuit\njusqu'a la ville. Elle cree meme un embouteillage,\nque voulez-vous faire ?"), heroPtr->getDescription() + std::string(" being chased by a goat"), std::string("Fuir"), std::string("Essayer d'arreter la chevre"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ...
    ee::Story *story70 = new ee::Story(std::string("Vous avez decide de caresser la chevre.\nElle vous suit jusqu'a la ville et finit\nmeme par creer un embouteillage. Que faites-vous ?"), heroPtr->getDescription() + std::string(" petting a goat in the moutain"), std::string("Vous fuyez"), std::string("Vous essayez d'arreter la chevre"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ...
    ee::Story *story75 = new ee::Story(std::string("Vous avez fui et vous retrouvez maintenant\nau bar. Que souhaitez vous boire ?"), heroPtr->getDescription() + std::string(" in a bar"), std::string("Une biere"), std::string("Du soda"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story76 = new ee::Story(std::string("Vous vous battez avec l'homme, et vous gagnez !\nIl retourne a sa voiture. Vous voyez alors\nune vieille dame se faire voler son sac.\nElle a l'air blessee ! Que faire ?"), heroPtr->getDescription() + std::string(" seeing an old lady getting hurt by someone"), std::string("Courir apres le voleur"), std::string("Aller aider la dame blessee"), heroPtr, badGuyPtr, ee::storyType::INGAME);
    ee::Story *story77 = new ee::Story(std::string("Vous essayez de discuter calmement,\nmais il vous frappe subitement et vous\nsuccombez a vos blessures."), heroPtr->getDescription() + std::string(" getting hit by an angry man in the highway"), std::string(""), std::string(""), heroPtr, badGuyPtr, ee::storyType::LOOSE);
    ...
    ee::Story *story79 = new ee::Story(std::string("Vous aidez la pauvre dame blessee,\net vous l'emmenez a l'hôpital. Vous venez\nde lui sauver la vie, vous etes un super-heros !"), heroPtr->getDescription() + std::string(" carrying an old lady to the hospital"), std::string(""), std::string(""), heroPtr, badGuyPtr, ee::storyType::WIN);
